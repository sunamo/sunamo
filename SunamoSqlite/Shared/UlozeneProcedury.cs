...
{
    public class UlozeneProcedury : IUlozeneProcedury
    {
        public static UlozeneProcedury ci = new UlozeneProcedury();
        ...
        public SQLiteCommand GetCmdInFormat(string f, List<int> nenahrazovat, params object[] p)
        {
            ...
            //return new SQLiteCommand(SH.Format2(f, p), DatabaseLayer.conn);
        }
        ...
        public string ReplaceValueOnlyOne(object p)
        {
            if (p != null)
            {
                ...
                {
                    // Mus�m vr�tit hned proto�e na konci mi to replacuje uvozovky
        ...
        {
            string[] sloupce = UlozeneProceduryI.ci.VratNazvySloupcuTabulky(tabulka);
            StringBuilder sb = new StringBuilder();
            sb.Append("(");
            foreach (String var in sloupce)
            {
        ...
        internal string GetColumnsWithoutBracets(string[] sloupce)
        {
