...
namespace YoutubeExtractor
{
    ...
    public class VideoDownloader : Downloader
    {
        ...
        /// Ač je metoda asynchronní, je po tom co jsem si vyzkoušel zajímavé že se chová synchronně. Teda, pokud ji zavoláte s await, bude asychnronní, pokud bez await, bude taky asynchronní ale vyvolá událost až jako poslední krok. Zřejmě za to může to že await je jen v usingu.
        /// </summary>
        ...
        public async override Task Execute()
        {
            ...
            using (WebResponse response = await request.GetResponseAsync())
            {
                ...
                {
                    using (FileStream target = await Task.Run<FileStream>(() => File.Open(this.SavePath, FileMode.Create, FileAccess.Write)))
                    {
