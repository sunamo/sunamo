...
public partial class MSColumnsDB : List<MSSloupecDB>
{
    ...
    public string GetTROfColumns()
    {
        ...
        if (this.Count == 0)
        {
            ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Nebyly nalezeny žádné sloupce");
        }
        sb.Append("(");
    ...
    public string GetNameTableRow(string nazevTabulky, string dbPrefix, out bool isDynamicTable, out string nazevTabulkyJC, out string dbPrefix2)
    {
        ...
        string nazevTabulkyCopy = SH.Copy(nazevTabulky);
        string niMethod = CSharpGenerator.AddTab(2, "ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod());");
...
ParseRow(o);");
        ...
        {
            ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Tabulka nemůže mít jen 1 sloupec.");
        }
        else
        {
    ...
    public string GetCsEntityView(string table, string dbPrefix, string nameOfVariable)
    {
        ...
        foreach (MSSloupecDB item in this)
        {
            ...
            switch (item.Type2)
            {
                ...
                case SqlDbType.DateTimeOffset:
                case SqlDbType.Timestamp:
                    ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Datový typ DateTimeOffset a Timestamp není podporován.");
                    return null;
                case SqlDbType.Real:
                case SqlDbType.Float:
                    nameOfVariable = nameOfVariable + "." + item.Name + ".ToString()";
                ...
                case SqlDbType.Binary:
                case SqlDbType.VarBinary:
                    ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Not supported convert binary data to string");
                    return null;
                case SqlDbType.Structured:
                    ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Strukturované datové typy nejsou podporovány.");
                    return null;
                case SqlDbType.Udt:
                    ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Univerzální datové typy nejsou podporovány.");
                    return null;
                case SqlDbType.Variant:
                    ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Variantní datové typy nejsou podporovány.");
                    return null;
                case SqlDbType.Xml:
                    ThrowExceptions.Custom(Exc.GetStackTrace(), type, Exc.CallingMethod(),"Xml datový typ není podporován");
                    return null;
                default:
                    break;
            }
