...
{
    internal class Mp3AudioExtractor : IExtractAudio
    {
        ...
        private void ParseMp3Frames(byte[] buffer)
        {
            ...
            while (length >= 4)
            {
                ...
                if (BitOperations.Read(ref header, 11) != 0x7FF)
                {
                    break;
                }

                mpegVersion = BitOperations.Read(ref header, 2);
                int layer = BitOperations.Read(ref header, 2);
                BitOperations.Read(ref header, 1);
                int bitRate = BitOperations.Read(ref header, 4);
                sampleRate = BitOperations.Read(ref header, 2);
                int padding = BitOperations.Read(ref header, 1);
                BitOperations.Read(ref header, 1);
                channelMode = BitOperations.Read(ref header, 2);
        ...
        private void WriteVbrHeader(bool isPlaceholder)
        {
            ...
            if (!isPlaceholder)
            {
                ...
                BitOperations.CopyBytes(buffer, 0, BigEndianBitConverter.GetBytes(header));
                BitOperations.CopyBytes(buffer, dataOffset, BigEndianBitConverter.GetBytes(0x58696E67)); // "Xing"
                BitOperations.CopyBytes(buffer, dataOffset + 4, BigEndianBitConverter.GetBytes((uint)0x7)); // Flags
