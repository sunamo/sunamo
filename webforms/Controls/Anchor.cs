    ...
    public override string Render(int actualRow, List<String[]> _dataBinding = null)
    {
        ...
        if (Delegate != null || DelegateInnerHtml != null)
        {
            if (actualRow != -1)
            {
                if (Delegate != null)
                {
                    hrefRender = Delegate.Invoke(_dataBinding[int.Parse(DelegateArgs.TrimStart(AllChars.cbl).TrimEnd(AllChars.cbr))][actualRow]);
                    if (hrefRender == null)
                    {
                        return "";
                    }
                ...
                {
                    innerHtml = DelegateInnerHtml.Invoke(_dataBinding[int.Parse(DelegateInnerHtmlArgs.TrimStart(AllChars.cbl).TrimEnd(AllChars.cbr))][actualRow]);
                }
                //return Render(hg);
            }
            else
