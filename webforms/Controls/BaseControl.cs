...
public abstract class BaseControl
{
    ...
    /// Znak { - počáteční
    /// </summary>
    public static char p = AllChars.cbl;
    /// <summary>
    /// Znak } - ukončovací
    /// </summary>
    public static char k = AllChars.cbr;
    ...
    protected string CallDelegate(HtmlGenerator hg, ref string src)
    {
        ...
        if (actualRow != -1)
        {
            ...
            List<string> s = null;
            string oddelovac = "";
            if (parametry.Contains(AllChars.comma))
            {
                s = SH.Split(parametry, AllChars.comma);
                oddelovac = AllStrings.comma;
            }
            else if (parametry.Contains(AllChars.pipe))
            {
                s = SH.Split(parametry, AllChars.pipe);
                oddelovac = AllStrings.pipe;
            }
            else
            {
            ...
            {
                if (item.StartsWith(AllStrings.cbl) && item.EndsWith(AllStrings.cbr))
                {
